...
namespace NoteEditor.Notes
{
    public class NoteObject : IDisposable
    {
        ...
        Color consecutiveNoteColor = new Color(255 / 255f, 65 / 255f, 0 / 255f);
        ...
        public void Init()
        {
            ...
            disposable.Add(noteType.Where(_ => !isSelected.Value)
                ...
                .Subscribe(noteType =>
                {
                    if (noteType == NoteTypes.Long)
                    {
                        noteColor_.Value = longNoteColor;
                    }
                    else if(noteType == NoteTypes.Consecutive )
                    {
                        noteColor_.Value = consecutiveNoteColor;
                    }
                    else if(noteType == NoteTypes.Single)
                    {
            ...
            disposable.Add(mouseDownObservable.Where(editType => editType == NoteTypes.Long || editType == NoteTypes.Consecutive )
                .Where(editType => editType == noteType.Value)
                .Subscribe(_ =>
                {
            ...
            var longNoteUpdateObservable = LateUpdateObservable
                .Where(_ => noteType.Value == NoteTypes.Long || noteType.Value == NoteTypes.Consecutive);
            ...
            disposable.Add(longNoteUpdateObservable
                ...
                .Merge(longNoteUpdateObservable
                    .Where(_ => EditState.NoteType.Value == NoteTypes.Long || noteType.Value == NoteTypes.Consecutive)
                ...
                .Select(nextPosition =>
                    ...
                    isSelected.Value || EditData.Notes.ContainsKey(note.next) && EditData.Notes[note.next].isSelected.Value ? selectedStateColor
                        ...
                        noteType.Value == NoteTypes.Long? longNoteColor : consecutiveNoteColor
                        : invalidStateColor)
                )
        ...
        public void SetState(Note note)
        {
            ...
            if (note.type == NoteTypes.Long || note.type == NoteTypes.Consecutive)
            {
